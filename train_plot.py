...
def get_gaussian_pyramid(image):
    dim, rows, cols = image.shape
    gaussian_pyramid = [image]
    while rows > 1 and cols > 1:
        image = pyramid_reduce(image, downscale=2, channel_axis=0)
...
def get_laplacian_pyramid(gaussian_pyramid):
    ...
    for i in range(len(gaussian_pyramid)-2, -1, -1):
        image = gaussian_pyramid[i] - resize(pyramid_expand(gaussian_pyramid[i+1], channel_axis=0), gaussian_pyramid[i].shape)
